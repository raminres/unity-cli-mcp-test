...
- **Inverted Stepped Pyramid Paddle & Dynamic Deflection**:
  ...
  - **Anti-Trap Ball Physics (Options A3 & B3)**:
    ...
    - **Ceiling Anti-Vertical Lateral Dispersion ($15^\circ$)**: When bouncing off the flat horizontal ceiling (`normal.y < -0.6f && Mathf.Abs(normal.x) < 0.5f`), `BallController.ApplyCeilingAntiVerticalDeflection()` mathematically enforces $|v_x| \ge \text{currentSpeed} \cdot \sin(15^\circ)$ biased away from center ($X = 0$), preventing balls from rebounding on straight vertical descent paths.
    - **Top Corner $45^\circ$ Chamfers**: Physical angled boundary wedges (`Chamfer_TopLeft` at $(-8.85, 22.85, 0)$ with $+45^\circ$ rotation and `Chamfer_TopRight` at $(8.85, 22.85, 0)$ with $-45^\circ$ rotation) redirect vertical corner balls into diagonal cross-arena trajectories. Configured in `SetupBlockBreakerScenes.cs` and guaranteed at runtime via `LevelGenerator.EnsureCornerChamfers()`.
...
- **Arena Dimensions**:
  - Top Wall: $Y = 24.25$, Left/Right Walls: $X = \pm 10.5$ (height $32.0$), Top Chamfers: $(\pm 8.85, 22.85)$, Kill Zone: $Y = -9.0$ ([KillZone.cs](file:///c:/Users/ramin/Desktop/Repos/unity-cli-mcp-test/Assets/Scripts/BlockBreaker/KillZone.cs)).

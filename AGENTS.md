...
- **Inverted Stepped Pyramid Paddle & Physical Geometry**:
  ...
  - **Optical Ray Deflection & Paddle Steering Formula**:
    ...
    - Preserves horizontal momentum (`rayAngleDeg = Mathf.Atan2(|inVelocity.y|, inVelocity.x) * Rad2Deg`), applies offset steering `steer = -hitOffset * 32f`, applies paddle momentum `clamp(-paddleVelocityX * 0.5f, -12°, +12°)`, strictly excludes vertical deadzone $[85^\circ, 95^\circ]$, clamped to $[25^\circ, 155^\circ]$.

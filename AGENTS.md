...
- **Lone Block Clutch Countdown & Option B Hyper-Beam Railgun**:
  ...
  - **Option B Hyper-Beam Railgun**: If the 12-second timer expires without hitting the block, the paddle engages emergency Railgun Overcharge. A wide vertical hyper-beam ($W \approx 3.2$, $H \approx 31$) surges progressively upward from the paddle deck to the arena ceiling over $0.65\text{s}$ (`beamSurgeDuration = 0.65f`), allowing the player to clearly track the leading beam edge as it ascends and slices through remaining bricks. Rendered via a dedicated URP gradient shader `Assets/Shaders/VFX_LaserHyperBeam.shader` on a foreground Quad ($Z = -0.3$), featuring a brilliant white-hot central core, electric pink inner glow, neon ruby outer aura with soft lateral falloff, solar amber muzzle flare at the paddle deck, and animated plasma energy ripple.
    - **Cross-Platform iOS Metal Guarantee**: Shaders without serialized scene materials are stripped by Unity's iOS build pipeline. To guarantee zero flat-red fallbacks on iOS, `Arcade/VFX_LaserHyperBeam` is registered in `GraphicsSettings.alwaysIncludedShaders`, serialized directly via `Assets/Materials/BlockBreaker/MI_LaserHyperBeam.mat` on `PaddleLaserController`, and textured via dedicated $256 \times 512$ RGBA asset `Assets/Textures/TX_LaserHyperBeam_Gradient.png` mapped to `_MainTex` and fallback `_BaseMap`.
  - **Dual-Cadence Level Clear Pacing & Celebratory Center Banner**:
    - **Celebratory Banner (`level-clear-banner`)**: Immediately upon clearing the final brick, a high-contrast golden neon center banner displays `"LEVEL CLEARED!"` with context-aware cyan subtext (`"STAGE COMPLETE!"`, `"FLAWLESS VICTORY!"`, or `"CLUTCH OVERCHARGE!"`) while the game remains cleanly in `GameState.Playing`.
    - **Dual Delay Cadence & Modal Separation**:
...
- **World Space Badges & Powerup Icons ([PowerupIconSet.cs](file:///c:/Users/ramin/Desktop/Repos/unity-cli-mcp-test/Assets/Scripts/BlockBreaker/PowerupIconSet.cs), [BlockBadge.cs](file:///c:/Users/ramin/Desktop/Repos/unity-cli-mcp-test/Assets/Scripts/BlockBreaker/BlockBadge.cs))**: Rendered via Unity 6 `PanelRenderer` in `WorldSpace` mode (`80px` fixed dimension, 100 PPU, clamped margins). All 7 powerup and modifier sprites (Expander, Bomb, Heart+, Shield, Multi-Ball, Multiplier, Laser) are managed and visually assignable in the Unity UI Inspector via ScriptableObject `Assets/Settings/SO_PowerupIcons.asset` and wired directly to `LevelGenerator` and `ArcadeUIManager`. Sprites are assigned directly in C# via `new StyleBackground(sprite)`, completely bypassing fragile `project://database/` USS URLs and guaranteeing 100% reliable rendering on iOS/iPad Metal builds.
